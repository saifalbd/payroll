    ...
    <div>
        <main class="container-fluid">
            <div class="d-flex justify-content-center mt-2">
                <h5 class="text-secondary font-weight-bold">Employee Attendance Sheet - {{currentMonthName}} {{currentYear}}</h5>
            </div>
    ...
    try{
        ...
        if(index !== -1){
            ...
            if(attIndex !== -1){
                ...
                employees.value[index].attendances[attIndex].in_outs = [{
                    time_in: attendanceForm.value.time_in,
                    time_out: attendanceForm.value.time_out,
                }];
            }
        }
...
const onMonthYearChange = async () => {
    makeDaysArray(selectedYear.value, selectedMonth.value);
    await fetchAttendances();
};

const makeDaysArray = (year: number, month: number) => {
    currentYear.value = year;
    currentMonth.value = month;
    currentMonthName.value = monthNames.value[month];

    const max_day = new Date(year, month + 1, 0).getDate();

    from_date.value = `${year}-${String(month + 1).padStart(2, '0')}-01`;
    to_date.value = `${year}-${String(month + 1).padStart(2, '0')}-${String(max_day).padStart(2, '0')}`;
